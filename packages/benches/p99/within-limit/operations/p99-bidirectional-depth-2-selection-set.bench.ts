 ...
 * with selectionSetDepth=2, the depth most TS2590-hit users will reach for first
 * (lightest dial-down that still allows `rel.subRel.scalar` paths).
 */

const selectionSet = ['name', 'spokes1.*', 'spokes2.*', 'spokes3.*'] as const;

bench(
  '10 models with bi-directional relationships - selectionSetDepth=2',
  async () => {
    const schema = a
      .schema({
        Hub: a.model({
